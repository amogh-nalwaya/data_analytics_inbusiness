...
def group_basket_stats(product_list, df_transactions, df_demographic):
    ...
    df_grouped_basket_merge = merging_sum_count_labels(df_grouped_basket, df_grouped_basket_count,
                                                       df_grouped_basket_sum)

    print("merging with demmographic data....")
    df_grouped_basket_merge = df_grouped_basket_merge.merge(df_demographic, on="household_key", how="left").reset_index(
        drop=True)
...
def get_grouped_basket_sum(df_grouped_basket):
    ...
    df_grouped_basket_sum.drop(['RETAIL_DISC', 'TRANS_TIME', 'COUPON_MATCH_DISC', 'START_DAY', 'END_DAY'], axis=1,
                               inplace=True)
    return df_grouped_basket_sum


def merging_sum_count_labels(df_grouped_basket, df_grouped_basket_count, df_grouped_basket_sum):
    df_grouped_basket_merge = df_grouped_basket_sum.merge(df_grouped_basket,
                                                          on=["household_key", "BASKET_ID"]).reset_index(drop=True)
    del df_grouped_basket
    del df_grouped_basket_sum

    df_grouped_basket_merge = df_grouped_basket_merge.merge(df_grouped_basket_count,
                                                            on=["household_key", "BASKET_ID"]).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    df_transactions = pd.read_csv('transaction_data.csv',
                                  dtype={'BASKET_ID': str, 'PRODUCT_ID': str, 'household_key': str, 'DAY': str})
    print("lenght of all transactions: " + str(len(df_transactions)))

    transactions_within_campaign, households_campaign_list = get_transactions_for_hh_within(df_transactions,
                                                                                            hh_start_dates,
                                                                                            product_list)
    ...
    df_grouped_basket['demo_missing'] = np.where(df_grouped_basket['INCOME_DESC'].isnull(), 1,
                                                 0)  # Creating one-hot for where demographic data is missing
    ...
    # trained_mlp = train_mlp(features_std, y, 3, 1, 20, 20)
    #
    ...
    #     {'household_key': pred_household_key, 'pred': pred_set_preds, 'pred_soft': pred_set_pred_prob})
    #
    ...
    pred_df = pd.read_csv('pred_df_51800000050.csv', dtype={'household_key': str})
    ...
    pred_df_w_camp_type.drop([col for col in pred_df_w_camp_type.columns if "DAY" in col], errors='ignore', axis=1,
                             inplace=True)
    ...
    for i in demographic_columns:
        ...
        layout = go.Layout(
            title='Purchase Probability Added by ' + i + ' Group',
            titlefont=dict(
                size=26
            ),
            xaxis=dict(
            ...
            yaxis=dict(
                title='Purchase Probability Mean',
                titlefont=dict(
                    family='sans serif',
                    size=22,
                    color='#000000'
