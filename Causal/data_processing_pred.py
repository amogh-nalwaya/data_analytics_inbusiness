...
def get_grouped_basket(product_list, trans_merge, df_demographic):
    ...
    #weeks to mailer
    df_grouped_basket_mailer = trans_merge.loc[trans_merge.groupby(['household_key', 'BASKET_ID'], sort=False)['WEEKS_TO_MAILER'].idxmin()]
    ...
    # Renaming columns
    df_grouped_basket_merge.rename(columns={"DAY_y":"DAY", "WEEKS_TO_MAILER_y":"WEEKS_TO_MAILER", "STORE_ID_x":"STORE_ID"}, inplace=True)
    
    #df_grouped_basket_merge.columns = ['household_key', 'BASKET_ID', 'STORE_ID', 'QUANTITY', 'SALES_VALUE', 'COUPON_DISC', 'CUSTOMER_PAID', 'label', 'DAY', 'COUNT', 'WEEKS_TO_MAILER']
    
    df_grouped_basket_merge = df_grouped_basket_merge.merge(df_demographic, on="household_key", how = "left").reset_index(drop=True)
    
    df_grouped_basket_merge['WEEKS_TO_MAILER_RECIP'] = df_grouped_basket_merge.apply(lambda row: 1/(row['WEEKS_TO_MAILER']+1), axis=1)
...
#    del trans_merge
    ...
    df_transactions[df_transactions.BASKET_ID == pred_set_indicator].head()
